...
# This script generates scripts to make cartographic projections in batches.
#
# Each batch contains only 10 timepoints to avoid running out of memory. It
# also creates a submit file batch_submit to run everything.
#
# Before running, edit the parameters below to match your current dataset. You
# need to copy them from TutorialIlastikDetectorSpherelikeFitter.m
#
# After that open a terminal and run this script using the number of timepoints
# as argument:
#
